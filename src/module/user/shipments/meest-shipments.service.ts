...
import { getShipmentActionFlags, mapMeestStatus } from './shipment-status.mapper.js';
...
@Injectable()
export class MeestShipmentsService {
  constructor(private readonly prisma: PrismaService) {}

  async createShipment(userId: number, dto: CreateMeestShipmentDto) {
    const postalService = await this.getOrCreatePostalService();

    await this.requireActiveConnection(userId, postalService.id);

    const ttn = `ME${Date.now()}${Math.floor(Math.random() * 1000)}`;

    await this.prisma.db.meestShipment.create({
      data: {
        userId,
        ttn,
  ...
  async getShipments(
    ...
    if (!connection) {
      ...
      throw new UnauthorizedException(
        'Meest connection is not active. Please connect or reconnect in your profile.',
      );
    }

    const rows = await this.prisma.db.meestShipment.findMany({
      where: { userId },
  ...
  async getShipmentDetail(userId: number, ref: string): Promise<ShipmentDetail> {
    ...
    const row = await this.prisma.db.meestShipment.findFirst({
  ...
  async deleteShipment(userId: number, ref: string): Promise<{ deletedAt: string }> {
    ...
    await this.prisma.db.meestShipment.delete({ where: { id: row.id } });
  ...
  private async getOrCreatePostalService() {
    return this.prisma.db.postalService.upsert({
      where: { slug: 'meest' },
      create: { name: 'Meest Express', slug: 'meest', isActive: true },
      update: {},
